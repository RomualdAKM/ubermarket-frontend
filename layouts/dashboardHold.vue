      ...
      <nav class="h-[calc(100vh-4rem)] overflow-y-auto py-4 px-3">
        ...
        <div class="space-y-1">
          ...
          <NuxtLink
            ...
            :to="getDashboardLink(productsRoute)" 
            class="sidebar-link group flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all"
            :class="[isSidebarOpen ? '' : 'justify-center', 'text-slate-600 hover:text-slate-900 hover:bg-white']"
            @click="closeSidebar"
          >
            ...
            <span :class="isSidebarOpen ? '' : 'lg:hidden'">
              {{ productsLabel }}
            </span>
          </NuxtLink>
        </div>
...
// Détection du type de boutique
const isDigitalShop = computed(
  () => currentShop.value?.product_type === 'digital'
)

// Route dynamique des produits
const productsRoute = computed(() => {
  switch (currentShop.value?.product_type) {
    case 'digital':
      return 'products/produits-digitaux'

    case 'physical':
      return 'products/produits'
...
//prévision autres types (service, booking, etc.)
...
/*const shopModules = computed(() => {
  switch (currentShop.value?.product_type) {
    case 'digital':
      return {
        productsPage: 'products/produits-digitaux',
        label: 'Produits digitaux'
      }

    case 'physical':
      return {
        productsPage: 'products/produits',
        label: 'Produits'
      }

    case 'service':
      return {
        productsPage: 'services/prestations',
        label: 'Prestations'
      }

    case 'booking':
      return {
        productsPage: 'reservations',
        label: 'Réservations'
      }

    case 'course':
      return {
        productsPage: 'formations',
        label: 'Formations'
      }

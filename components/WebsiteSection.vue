  ...
  <section :style="sectionStyle" class="website-section">
    ...
    <template v-else-if="section.type === 'video'">
      <div :style="{ maxWidth: section.style?.maxWidth || '900px', margin: '0 auto', padding: '0 1rem' }">
        ...
        <p v-if="section.content?.description" class="mt-4 text-sm text-center opacity-70" :style="{ color: section.style?.textColor }">
          {{ section.content.description }}
        </p>
      </div>
    </template>
      ...
      <div :style="{ maxWidth: section.style?.maxWidth || '600px', margin: '0 auto', padding: '0 1rem', textAlign: 'center' }">
        ...
        <div class="flex justify-center gap-4 md:gap-6">
          <div v-for="unit in countdownUnits" :key="unit.label" class="text-center">
            <div class="text-4xl md:text-6xl font-bold" :style="{ color: section.style?.textColor }">{{ unit.value }}</div>
            <div class="text-xs uppercase tracking-wider opacity-60 mt-1" :style="{ color: section.style?.textColor }">{{ unit.label }}</div>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="section.type === 'social'">
      <div :style="{ maxWidth: section.style?.maxWidth || '400px', margin: '0 auto', padding: '0 1rem', textAlign: 'center' }">
        <h3 v-if="section.content?.title" class="text-lg font-semibold mb-6" :style="{ color: section.style?.textColor }">
          {{ section.content.title }}
        </h3>
...
// Countdown timer
const countdownValues = ref({ days: 0, hours: 0, minutes: 0, seconds: 0 })
let countdownInterval: ReturnType<typeof setInterval> | null = null

const countdownUnits = computed(() => [
  { label: 'Jours', value: String(countdownValues.value.days).padStart(2, '0') },
  { label: 'Heures', value: String(countdownValues.value.hours).padStart(2, '0') },
  { label: 'Minutes', value: String(countdownValues.value.minutes).padStart(2, '0') },
  { label: 'Secondes', value: String(countdownValues.value.seconds).padStart(2, '0') }
])

const updateCountdown = () => {
  if (props.section.type !== 'countdown' || !props.section.content?.endDate) return
  
  const endDate = new Date(props.section.content.endDate).getTime()
...
onMounted(() => {
  if (props.section.type === 'countdown') {
    updateCountdown()
